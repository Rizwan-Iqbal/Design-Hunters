	...
	<div class="row">
		 	<?php //******************//
				...
				while($best_wp->have_posts()): $best_wp->the_post();					
					...
					if($google_plus!=''){
						$gp='<a href="'.$google_plus.'" class="social-icon" target="_blank"><i class="fab fa-instagram"></i></a> ';
					}
					if($linkedin!=''){
				...
				?>		
					...
					<div class="col-lg-<?php echo esc_html($settings['team_columns']);?> col-md-6">	

						<div class="team-item">
							<div class="team-inner-wrap">
								<div class="image-wrap">
									<?php if('none_style' == $settings['team_grid_popup']){ ?>
										<?php the_post_thumbnail($settings['thumbnail_size']); ?>										
									<?php } else { ?>
										<?php if($settings['team_grid_popup']=='popup'): ?>
								  		<a class="pointer-events" href="#rs_popupBox2_<?php echo esc_attr($unique);?>" data-effect="mfp-zoom-in">
									    <?php else: ?>
				...
				<?php
					if($facebook!=''){
						$fb_popup='<a href="'.$facebook.'" target="_blank" class="social-icon" '.$icon_style.'><i class="fab fa-facebook-f"></i></a> ';
					}
					if($twitter!=''){
						$tw_popup='<a href="'.$twitter.'" target="_blank" class="social-icon" '.$icon_style.'><i class="fab fa-twitter"></i></a>';
					}
					if($google_plus!=''){
						$gp_popup='<a href="'.$google_plus.'" target="_blank" class="social-icon" '.$icon_style.'><i class="fab fa-instagram"></i></a> ';
					}
					if($linkedin!=''){
						$ldin_popup='<a href="'.$linkedin.'" target="_blank" class="social-icon" '.$icon_style.'><i class="fab fa-linkedin-in"></i></a>';
					}
				?>

				<div id="rs_popupBox2_<?php echo esc_attr($unique);?>" class="rspopup_style1  mfp-with-anim mfp-hide" <?php echo wp_kses_post($popup_background);?>>
					<div class="row">
						<div class="col-md-5">
